...
if(isset($_GET['id']) && !empty($_GET['id'])){
	$qry = $conn->query("SELECT id, fullname, niv_sp FROM `major` where id = '{$_GET['id']}'");
	...
	<div class="card-body">
		<form action="" id="major-form">			
			...
			<div class="form-group">
				...
				<textarea name="niv_sp" id="niv_sp" cols="30" rows="3" class="form-control" style="resize: none" required><?php echo isset($niv_sp) ? $niv_sp : ''; ?></textarea>
			</div>
		</form>
	</div>
...
<script>
	$(document).ready(function(){
		$('#major-form').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f=save_major",
				method:'POST',
				data:$(this).serialize(),
				error:err=>{
					console.log(err)
