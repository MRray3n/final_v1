...
if(isset($_GET['id']) && !empty($_GET['id'])){
	$qry = $conn->query("SELECT id, vacation_name,vacation_date  FROM `vacation` where id = '{$_GET['id']}'");
...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?php echo isset($_GET['id']) ? "Update ": "Create New " ?>vacances</h3>
	</div>
	<div class="card-body">
		<form action="" id="vacation-form">			
			...
			<div class="form-group">
				...
				<textarea name="vacation_name" id="vacation_name" cols="30" readonly rows="3" class="form-control" style="resize: none" required><?php echo isset($vacation_name) ? $vacation_name : ''; ?></textarea>
			</div>
		</form>
	</div>
...
<script>
	$(document).ready(function(){
		$('#vacation-form').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f=update_vacation",
				method:'POST',
				data:$(this).serialize(),
				error:err=>{
					console.log(err)
